...
int set_directory(rados_t cluster, char *dirName,rados_ioctx_t *io_ctx){
	...
	error =  rados_ioctx_create(cluster, dirName, io_ctx);
	if(error < 0)
		fprintf(stderr, "[set_directory/ceph_lib.c] Erro ao criar contexto de io: %s!\n", strerror(-error));
...
int get_object_size(rados_ioctx_t io_ctx,char *obj_id,uint64_t *len_bucket,time_t *mtime){
	...
	if(error < 0){
		fprintf(stderr, "[get_object_size/ceph_lib.c] Não foi possível localizar o arquivo: %s\n", strerror(-error));
